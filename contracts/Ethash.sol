...
contract SHA3_512 {
    ...
    function keccak_f(uint256[25] memory A) private pure returns (uint256[25] memory) {
        uint256[25] memory B;
        uint256[5] memory C;
        uint256[5] memory D;
    ...
    function sponge(uint256[9] memory M) internal pure returns (uint256[16] memory) {
...
contract Ethash is SHA3_512 {
    uint256 constant EPOCH_LENGTH = 30000; // blocks per epoch

    constructor() {}

    function fnv(uint256 v1, uint256 v2) internal pure returns (uint256) {
        return ((v1 * 0x01000193) ^ v2) & 0xFFFFFFFF;
    }

    function computeCacheRoot(
        uint256 index,
        uint256 indexInElementsArray,
        uint256[] memory elements,
        uint256[] memory witness,
        uint256 branchSize
    ) private pure returns (uint256) {
        uint256 leaf = computeLeaf(elements, indexInElementsArray) & 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF;
        ...
        uint256 witnessIndex = indexInElementsArray * branchSize;
        ...
        for (depth = 0; depth < branchSize; depth++) {
            ...
            leaf = uint256(keccak256(abi.encodePacked(left, right))) & 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF;
            assembly {
                index := div(index, 2)
            }
    ...
    function toBE(uint256 x) internal pure returns (uint256) {
        uint256 y = 0;
    ...
    function computeSha3(uint256[16] memory s, uint256[8] memory cmix) internal pure returns (uint256) {
        uint256 s0 =
            s[0] +
                s[1] *
                (2**32) +
                ...
                s[3] *
                (2**96) +
                (s[4] + s[5] * (2**32) + s[6] * (2**64) + s[7] * (2**96)) *
                (2**128);
        ...
        uint256 s1 =
            s[8] +
                ...
                s[11] *
                (2**96) +
                (s[12] + s[13] * (2**32) + s[14] * (2**64) + s[15] * (2**96)) *
                (2**128);
        ...
        uint256 c =
            cmix[0] +
                ...
                cmix[3] *
                (2**96) +
                (cmix[4] + cmix[5] * (2**32) + cmix[6] * (2**64) + cmix[7] * (2**96)) *
                (2**128);

        /* god knows why need to convert to big endian */
        return uint256(keccak256(abi.encodePacked(toBE(s0), toBE(s1), toBE(c))));
    }

    function computeLeaf(uint256[] memory dataSetLookup, uint256 index) internal pure returns (uint256) {
        return
            uint256(
                keccak256(
                    abi.encodePacked(
    ...
    function computeS(uint256 header, uint256 nonceLe) internal pure returns (uint256[16] memory) {
        uint256[9] memory M;

        header = reverseBytes(header);

        M[0] = uint256(header) & 0xFFFFFFFFFFFFFFFF;
        header = header / 2**64;
        M[1] = uint256(header) & 0xFFFFFFFFFFFFFFFF;
        header = header / 2**64;
        M[2] = uint256(header) & 0xFFFFFFFFFFFFFFFF;
        header = header / 2**64;
        M[3] = uint256(header) & 0xFFFFFFFFFFFFFFFF;
    ...
    function reverseBytes(uint256 input) internal pure returns (uint256) {
        uint256 result = 0;
        for (uint256 i = 0; i < 32; i++) {
            result = result * 256;
            result += input & 0xff;
    ...
    struct EthashCacheOptData {
        uint256[512] merkleNodes;
        uint256 fullSizeIn128Resultion;
        uint256 branchDepth;
    }

    mapping(uint256 => EthashCacheOptData) epochData;

    function isEpochDataSet(uint256 epochIndex) public view returns (bool) {
        return epochData[epochIndex].fullSizeIn128Resultion != 0;
    }

    event SetEpochData(address indexed sender, uint256 error, uint256 errorInfo);

    function setEpochData(
        uint256 epoch,
        uint256 fullSizeIn128Resultion,
        uint256 branchDepth,
        uint256[] memory merkleNodes,
        uint256 start,
        uint256 numElems
    ) public {
    ...
    function getMerkleLeave(uint256 epochIndex, uint256 p) internal view returns (uint256) {
        uint256 rootIndex = uint256(p >> epochData[epochIndex].branchDepth);
        uint256 expectedRoot = epochData[epochIndex].merkleNodes[(rootIndex / 2)];
    ...
    function hashimoto(
        ...
        uint256[16] memory s;
        uint256[32] memory mix;
        uint256[8] memory cmix;

        uint256[2] memory depthAndFullSize =
            [epochData[epochIndex].branchDepth, epochData[epochIndex].fullSizeIn128Resultion];
        ...
        s = computeS(uint256(header), nonceLe);
        for (i = 0; i < 16; i++) {
            assembly {
        ...
        for (i = 0; i < 64; i++) {
            uint256 p = fnv(i ^ s[0], mix[i % 32]) % depthAndFullSize[1];

            if (
                computeCacheRoot(p, i, dataSetLookup, witnessForLookup, depthAndFullSize[0]) !=
                getMerkleLeave(epochIndex, p)
            ) {
                // PoW failed
        ...
        uint256 result = computeSha3(s, cmix);
    ...
    function verifyPoW(
        ...
        uint256 nonce,
        uint256 difficulty,
        uint256[] calldata dataSetLookup,
        uint256[] calldata witnessForLookup
    ) external view returns (uint256, uint256) {
        // verify ethash
        uint256 epoch = blockNumber / EPOCH_LENGTH;
        uint256 ethash = hashimoto(rlpHeaderHashWithoutNonce, nonce, dataSetLookup, witnessForLookup, epoch);
